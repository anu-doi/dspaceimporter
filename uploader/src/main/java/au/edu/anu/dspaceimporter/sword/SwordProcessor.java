package au.edu.anu.dspaceimporter.sword;
...
import au.edu.anu.dspaceimporter.sword.task.AddToMediaResourceTask;
import au.edu.anu.dspaceimporter.sword.task.ChangeInProgressTask;
import au.edu.anu.dspaceimporter.sword.task.DepositTask;
import au.edu.anu.dspaceimporter.sword.task.Md5CalcTask;
import au.edu.anu.dspaceimporter.sword.task.ReplaceTask;
...
//						bitstream = createInputStream(bitstreamInfo.getFile());
	...
	private SWORDCollection getCollectionNamed(ServiceDocument sd, String collectionName) throws WorkflowException {
		...
		if (collectionName != null) {
			if (collectionName.matches("^\\d+/\\d+$")) {
				for (SWORDWorkspace iWorkspace : sd.getWorkspaces()) {
					for (SWORDCollection iColl : iWorkspace.getCollections()) {
						if (iColl.getHref().toASCIIString().endsWith("/" + collectionName)) {
							collection = iColl;
							break;
						}
					}
